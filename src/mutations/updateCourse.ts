...
export const updateCourse = async (_parent, args: { id: number, input: UpdateCourseInput }) => {
    ...
    const [result] = await connection.execute<ResultSetHeader>(`UPDATE courses SET ${fieldsToUpdate.join(', ')} WHERE id = ?`, updateValues)

    if (result.affectedRows === 0) {
        throw new GraphQLError('no matching course found to update')
    }

    const [rows] = await connection.execute('SELECT * FROM courses WHERE id = ?', [args.id])
